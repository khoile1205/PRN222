...
namespace BussinessLayer.Services
{
    public class ShiftStaffService : IShiftStaffService
    {
        ...
        public async Task<PaginationResult<ShiftStaff>> GetShiftRequestsByStaffId(string staffId, int month, int year, int pageNumber = 1, int pageSize = 10)
        {
            var listShiftStaff = await _shiftStaffPaginationRepository.GetPaginatedAsync(
                        pageNumber: pageNumber,
                        pageSize: pageSize,
                        filter: ((r) => (r.StaffId == staffId && r.ShiftDate.Month == month && r.ShiftDate.Year == year)),
